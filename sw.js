// SHOMER Nature Service Worker v1
// Offline support + Push notifications for nature & coast emergency alerts
...
const CACHE_NAME = 'shomer-nature-v1-cache';
const urlsToCache = [
  './',
  'index.html',
  'nature.html',
  ...
  if (req.mode === 'navigate') {
    event.respondWith(
      fetch(req)
        ...
        .catch(() => caches.match(req).then(r => r || caches.match('nature.html')))
    );
    return;
  }
...
// Push notification handler (for SOS / nature & coast emergency alerts)
self.addEventListener('push', event => {
  const data = event.data ? event.data.json() : {};
  const title = data.title || 'SHOMER Nature — Safety Alert';
  const options = {
    body: data.body || 'A hazard was reported nearby. Tap to open the map.',
    icon: 'logo-192.png',
    badge: 'logo-192.png',
    vibrate: [200, 100, 200],
    tag: data.tag || 'shomer-nature-alert',
...
self.addEventListener('notificationclick', event => {
  ...
  if (event.action === 'call-100') {
    ...
    clients.openWindow('nature.html');
  }
});
